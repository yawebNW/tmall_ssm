...
public class ProductImageController {
  ...
  public String add(UploadFile uploadFile, ProductImage productImage, HttpSession session) throws IOException {
  ...
  private void singleImageProcess(HttpSession session,MultipartFile image, int id) throws IOException {
    ...
    resizeImageFile(file, "img/productSingle_small", 56, session.getServletContext());
    resizeImageFile(file, "img/productSingle_middle", 217, session.getServletContext());
  }


  private void resizeImageFile(File srcFile, String imageFolderPath, int size, ServletContext context) {
    ...
    ImageUtil.resizeImage(srcFile, size, size, file);
  }
